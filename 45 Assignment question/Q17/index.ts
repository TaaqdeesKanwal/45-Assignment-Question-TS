...
let guests=['Sakina','Abdullah','Taskeen','Sehar','Saifullah','Rubina', 'Abdul Moiz'
  ]
  console.log(`${guests}! New dinner table won’t arrive in time for the dinner, 
  so i have to invite only two guests. `)
  while(guests.length > 2){
    let removeGuest=guests.pop()
    console.log(`Dear ${removeGuest} !Sorry for cancellation of invitation`)
  }
  guests.map((item)=>(
